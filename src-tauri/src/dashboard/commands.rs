...
/// when no flag fits (e.g. a Team-view PR with no relation row of its own).
...
fn resolve_pr_route_metadata(
    ...
    let view = if row.archived_at.is_some() {
        ...
        // Team-view PR with no relation row, or a stale notification whose
...
mod tests {
    ...
    fn falls_back_to_authored_when_no_relation_row_exists() {
        // Team-view path: the PR sits in a team-tracked repo but the viewer
