...
2. **Sync.** A Rust worker polls per account on the configured interval (default 5min, range 30s–10min). Each poll uses ETag / `If-Modified-Since` conditional requests; per-resource ETags live in SQLite. Failures and rate-limit hits are isolated per account. See ADR [0004](../../docs/adr/0004-sync-polling-with-etag.md).
